...
void MarketDataAdapter::Update(Security::IdType id, double price, Qty size,
...
static inline void UpdateVolume(Qty qty, MarketData::Trade* t) {
...
void MarketDataAdapter::Update(Security::IdType id, double last_price,
                               Volume volume, double open, double high,
                               double low, double vwap) {
  auto& md = (*md_)[id];
  Volume d = volume - md.trade.volume;
  if (d <= 0) return;
